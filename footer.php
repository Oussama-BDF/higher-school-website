<?php
/**
 * The footer for EST-SB Theme.
 * 
 * Contains the footer template and the scripts js
 * 
 * @package EST-SB
 */
?>
    <!-- Start The Footer -->
    <div class="footer pt-5 pb-5 text-white-50 text-center text-md-start">
      <div class="container">
        <div class="row">

          <div class="col-md-6 col-lg-3">
            <h5 class="dropdown-list text-light" data-bs-target="#collapse1" aria-expanded="false" aria-controls="collapse1">L'etablissement</h5>
            <ul class="collapse-list list list-unstyled lh-lg" id="collapse1">
              <li>Adresse : Sidi Bennour</li>
              <li>Tel : 06 06 06 06 06</li>
              <li>Fax : 06 06 06 06 06</li>
              <li>Email : estsb@ucd.ac.ma</li>
              <li>Web : www.estsb.ucd.ac.ma</li>
            </ul>
          </div>

          <div class="col-md-6 col-lg-3">
            <h5 class="dropdown-list text-light" data-bs-target="#collapse2" aria-expanded="false" aria-controls="collapse2">L'université</h5>
            <ul class="collapse-list list list-unstyled lh-lg" id="collapse2">
              <li>Adresse : Avenue Jabran Khalil</li>
              <li>Tel : 06 06 06 06 06</li>
              <li>Fax : 06 06 06 06 06</li>
              <li>Email : communication@ucd.ac.ma</li>
              <li>Web : www.ucd.ac.ma</li>
            </ul>
          </div>

          <div class="col-md-6 col-lg-3">
            <h5 class="text-light">Suivez Nous Sur</h5>
            <ul class="net-list list d-flex list-unstyled lh-lg">
              <li>
                <a class="d-block" href="#">
                  <i class="mi10 fa-brands fa-facebook-f"></i>
                  <span class="mi10">Facebook</span>
                </a>
              </li>
              <li>
                <a class="d-block" href="#">
                  <i class="mi10 fa-brands fa-x-twitter"></i>
                  <span class="mi10">Twitter</span>
                </a>
              </li>
              <li>
                <a class="d-block" href="#">
                  <i class="mi10 fa-brands fa-linkedin-in"></i>
                  <span class="mi10">Linkedin</span>
                </a>
              </li>
              <li>
                <a class="d-block" href="#">
                  <i class="mi10 fa-brands fa-instagram"></i>
                  <span class="mi10">Instagram</span>
                </a>
              </li>
              <li>
                <a class="d-block" href="#">
                  <i class="mi10 fa-brands fa-youtube"></i>
                  <span class="mi10">Youtube</span>
                </a>
              </li>
            </ul>
          </div>

          <div class="col-md-6 col-lg-3 order1">
            <div class="info">
              <img width=328px src="<?php echo get_template_directory_uri()?>/assets/icon/est-ucd-white.svg" alt="" class=" img-fluid mb-4" />
              <p class="list mb-5">
                ESTSB L'Ecole Supérieure de Technologie Sidi Bennour (ESTSB) est un établissement public d'enseignement supérieur à finalité de formation des Techniciens Supérieurs.
              </p>
            </div>
          </div>

        </div>
      </div>
      <div class="copyright">
        Copyright 2023 <span>ESTSB</span> | Tous les droits sont réservés | &copy; Oussama Boudafdafa |
      </div>
      <!-- End The Footer -->

    </div>
    <?php wp_footer(); ?>
  </body>   
</html>